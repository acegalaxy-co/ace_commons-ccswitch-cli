# 📒 SỔ NĂNG LỰC — "đã từng LÀM cái này chưa? bản TỐT NHẤT ở đâu?"

> 🎯 **HỌC CHÉO** — TRƯỚC khi dựng 1 việc LẶP LẠI (auth, deploy, RBAC, CMS, thanh toán, feature-flag…): liếc đây xem có BẢN CHUẨN chưa → **TÁI DÙNG, đừng làm lại từ đầu**.
> 🧩 **2 LOẠI:** *năng lực* (`capability:` = cách DỰNG tính năng) + 🔧 *đồ nghề chạy được* (`cach-chay:` = công cụ có sẵn gọi chạy NGAY: trình quét, script, nút bấm).
> ⚙️ **TỰ IN** từ `capability:`/`cach-chay:`/`do-tin:` của mảnh (đừng sửa tay giữa 2 marker). Cập: dựng/cải tiến xong → set tag cho mảnh → `node tools/so-nang-luc.mjs --write` (hoặc memory-doctor đầu phiên tự làm tươi).
...
<!-- in lúc 2026-08-13 06:27 · quét 1 mảnh · 1 có 'capability:' · 1 năng lực · 0 đồ nghề -->
