...
`claude` / `deepseek` dùng **CÙNG base URL** `https://9router.proxy.com/v1` qua 9router **và CÙNG 1 token** (điền giống nhau vào cả 2 profile); chỉ khác block `ANTHROPIC_DEFAULT_*_MODEL` (prefix `cc/` vs `ds/`). Vì chung 1 router, router chết = cả 2 chết → fallback duy nhất là `subscription`.
