...
Đổi nhanh endpoint auth của **Claude Code** giữa **9router** (mặc định), **local router**, và **Anthropic direct** — chỉ thay block `env` trong `~/.claude/settings.json`, không đụng phần còn lại (hooks, permissions...).

| Profile | Endpoint | Vai trò |
|---|---|---|
| **`9router`** | `https://9router.acegalaxy.co/v1` | ⭐ **DEFAULT** — remote router (multi-model, key riêng) |
| `local` | `http://127.0.0.1:20128/v1` | Fallback 1 — router chạy local máy bạn (:20128) |
| `original` | `https://api.anthropic.com` | Fallback 2 — nối thẳng Anthropic (cần `ANTHROPIC_API_KEY` thật) |
...
profiles: 9router local original
```
...
Với `9router` và `local`, model **phải** có prefix `cc/`:
