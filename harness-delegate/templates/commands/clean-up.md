...
5. Loại trừ vùng cấm trước khi scan:
   ...
   - `node_modules/`
   - `.git/`
   - `.worktrees/` khi grep/find source, trừ khi scope là `worktree`
...
Mục tiêu: dọn `.worktrees/` đúng rule, tránh xoá nhầm worktree hợp lệ.
...
3. `worktrees`: `.worktrees/`, stale registered worktree, orphan dirs.
