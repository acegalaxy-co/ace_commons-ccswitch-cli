...
#   HARNESS_GROUP_RULES          y/n — rules: common/ (8 invariant guardrails, always overwrite) + project/ (git-workflow, skill-superpowers — kept if exist), no prompt (default: Y)
...
SEL_SUBAGENTS=0; SEL_GUARD=0; SEL_QUALITY=0; SEL_SESSIONLIMIT=0; SEL_COMMANDS=0; SEL_SKILLS=0; SEL_RULES=0; SEL_GITHOOKS=0
...
- ⭐⭐⭐ Global \`[[token-budget]]\` — **P0**: context-window budget; hook \`.claude/hooks/check-session-limit.sh\`
...
- [.claude/rules/common/](.claude/rules/common/) — invariant guardrails (secret, vault, budget, orchestrator, delegate, git, red-flags, rule-loading). Managed by harness install.sh: **overwrite** khi re-sync — KHÔNG sửa trực tiếp trong project (sửa upstream ở harness repo).
