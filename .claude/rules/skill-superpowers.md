...
Overlap đã có → dùng bản repo, KHÔNG dựng song song: worktree/code-review/verify → [[git-workflow]], `/code-review`, `/verify`, `/push-to-github`. Delegate/subagent → [[orchestrator]] (KHÔNG dùng subagent-driven-dev generic của superpowers).
...
Trước khi báo xong: chạy thật, quan sát hành vi (không chỉ "code trông đúng"). Dùng `/verify`. Mất verify step = chưa xong.
