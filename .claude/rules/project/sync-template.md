---
name: sync-template
description: Sửa xong bất kỳ harness surface LIVE (.claude/ + scripts/delegate/ + .git/hooks/pre-push) phải mirror ngược vào harness-delegate/templates/ — template là source-of-truth để install.sh cài sang project khác. LAZY, load khi chạm harness surface hoặc template.
status: live
updated: 2026-07-27
paths:
  - ".claude/**"
  - "harness-delegate/**"
  - "scripts/delegate/**"
metadata:
  type: reference
---

# Sync live harness → template (BẮT BUỘC)

`harness-delegate/templates/` là **source-of-truth** để `install.sh` cài harness sang project khác. Live `.claude/` (+ `scripts/delegate/`, `.git/hooks/pre-push`) chỉ là bản đã cài của repo này. Sửa live mà không mirror → template stale → mọi project cài sau nhận bản cũ.

**Quy tắc:** sửa/thêm/xoá bất kỳ file harness LIVE → mirror ngược vào template **cùng commit**. KHÔNG để 2 bên diverge qua session.

## Mapping (live ↔ template)

| Live | Template |
|---|---|
| `.claude/hooks/*.sh` | `harness-delegate/templates/hooks/*.sh` |
| `.claude/agents/*.md` | `harness-delegate/templates/agents/*.md` |
| `.claude/commands/*.md` | `harness-delegate/templates/commands/*.md` |
| `.claude/skills/<name>/` | `harness-delegate/templates/skills/<name>/` |
| `.claude/rules/common/*.md` | `harness-delegate/templates/rules/common/*.md` |
| `.claude/rules/project/*.md` | `harness-delegate/templates/rules/project/*.md` |
| `scripts/delegate/*` | `harness-delegate/templates/scripts/delegate/*` |
| `.git/hooks/pre-push` | `harness-delegate/templates/git-hooks/pre-push` |

## Reverse-bake token (QUAN TRỌNG — không copy thô)

Install-time `install.sh` thay `@@TOKEN@@` → giá trị repo. Mirror ngược phải **đảo lại**: giá trị baked của repo này → `@@TOKEN@@`. Copy thô = leak giá trị ccswitch vào template dùng chung.

| Baked (live repo) | Token (template) |
|---|---|
| `dev` (working branch) | `@@BRANCH@@` |
| `ccswitch-cli-claude` (slug) | `@@PROJECT_SLUG@@` |
| core-dir literals (src…) | `@@CORE_DIRS_*@@` (YAML/CASE/ALT/HUMAN) |
| test command | `@@TEST_CMD@@` |
| model default từ `.env` | `@@GEMINI_MODEL_DEFAULT@@` / `@@CODEX_MODEL_DEFAULT@@` / `@@DEEPSEEK_MODEL_DEFAULT@@` |

Token đầy đủ: `grep -rhoE '@@[A-Z_]+@@' harness-delegate/templates | sort -u`.

## Overwrite policy (khớp install.sh)

- `templates/rules/common/` — install **overwrite** khi re-sync → sửa nội dung guardrail chỉ ở template, KHÔNG sửa live trực tiếp (live sẽ bị đè lần cài sau).
- `templates/rules/project/` — install **giữ nguyên** nếu file đã tồn tại ở target → rule riêng repo an toàn ở live, vẫn mirror để project mới có bản seed.

## Verify sau mirror

```bash
# Không còn giá trị repo-specific lọt vào template
grep -rn "ccswitch-cli-claude" harness-delegate/templates && echo "LEAK ↑" || echo "clean"
# Installer chạy all-defaults không lỗi (dry run)
( cd harness-delegate && bash install.sh </dev/null >/dev/null 2>&1 ) && echo "install OK" || echo "install FAIL"
```

## Tránh

- ❌ Sửa live hook/rule/skill rồi quên template → drift âm thầm.
- ❌ Copy live → template không reverse-bake (leak `dev`/slug/model vào bản dùng chung).
- ❌ Sửa `common/` ở live (bị đè lần cài sau) — sửa ở template.
