...
setup() {
  ...
  cp "$ROOT/ai-proxy/kimi-anthropic-adapter.py" "$HOME/.claude/kimi-anthropic-adapter.py"
...
@test "apply kimi writes local adapter env block" {
  run "$CC" kimi
  [ "$status" -eq 0 ]
...
@test "update does not sync router host+key into kimi" {
  ...
  [[ "$kimi_token" == *"<your-kimi-key>"* ]]
}

@test "update from kimi is rejected because kimi has separate key" {
  run "$CC" update kimi
  [ "$status" -ne 0 ]
  [[ "$output" == *"update only syncs router profiles"* ]]
}
