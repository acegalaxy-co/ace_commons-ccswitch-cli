...
.DESCRIPTION
  ...
  Profiles (priority order):
    9router  (DEFAULT)  https://9router.proxy.com/v1   — remote router
...
.EXAMPLE
  ...
  ccswitch fallback       # first healthy profile: 9router -> original
#>
[CmdletBinding()]
param(
...
switch ($Command) {
  ...
  "fallback" {
    ...
    # guaranteed SAFE-HARBOR terminal: apply even if its probe ≠ 200, so Claude
    # never stays stuck on a dead router. Probe is advisory (can false-negative).
    foreach ($p in @("9router")) {
      $c = Test-Profile $p
  ...
  default { Die "usage: ccswitch [9router|original|check|fallback|clear|status]" }
}
